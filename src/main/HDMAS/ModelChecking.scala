...
class ModelChecking extends Cloneable{
  //val y1:IVariable = IVariable(1,Nat)
  //val y2:IVariable = IVariable(2,Nat)
  ...
  def PrF(M:HDMAS_Structure,s:State,t1:ITerm,t2:ITerm,Q:mutable.Set[State]):IFormula = {
    ...
    //val agNum:Int = M.getAgNum
    val actionSet:mutable.Set[Action] = M.getActionSet
    //val actionNum: Int = actionSet.size
    ...
    //actionSet.foreach(x => controllableList = controllableList :+ new ConstantTerm(s"k_${x.getAction}"))
    //actionSet.foreach(x => uncontrollableList = uncontrollableList :+ new ConstantTerm(s"l_${x.getAction}"))

    /*
    actionSet.foreach(x => {
      controllableList = controllableList :+ IVariable(100+c,Nat)
      c = c+1
    })
    */
    ...
    /*
    actionSet.foreach(x => {
      uncontrollableList = uncontrollableList :+ IVariable(200+u,Nat)
      u = u+1
    })
  ...
  /*
  def pfixPrF(M:HDMAS_Structure,s:State,t1:ITerm,t2:ITerm,Q:mutable.Set[State],pre:String,newt1:IConstant,newt2:IConstant):IFormula = { //y1 y2为
    import IExpression._
    ...
    */
    ...
    val sumK:IFormula = sum(controllableList) === newt1
    val sumL:IFormula = sum(uncontrollableList) === newt2
    ...
    if (formula != IExpression.Boolean2IFormula(false)) {
      for (i <- 1 to actionSize){
        // sub += (new ConstantTerm(s"x_$i") -> (new ConstantTerm(s"k_$i") + new ConstantTerm(s"l_$i")))
        formula = ConstantSubstVisitor(formula, Map(new ConstantTerm(s"x_$i") -> (new ConstantTerm(s"k_$i") + new ConstantTerm(s"l_$i"))))
      }
    }
    ...
    pre match {
      ...
      case "Exists" =>
        val part = (andL & sumL) ==> formula
        val part1 = quanConsts(Quantifier.ALL,uncontrollableList,part)
        val part2 = quanConsts(Quantifier.EX,controllableList,andK & sumK & part1)
        quanConsts(Quantifier.EX,List(newt1.c),part2)

      case "Forall" =>
        val part = (andL & sumL) ==> formula
        val part1 = quanConsts(Quantifier.ALL,uncontrollableList,part)
        val part2 = quanConsts(Quantifier.ALL,List(newt2.c),part1)
        quanConsts(Quantifier.EX,controllableList,andK & sumK & part2)

      case "EF" =>
        val part = (andL & sumL) ==> formula
        val part1 = quanConsts(Quantifier.ALL,uncontrollableList,part)
        val part2 = quanConsts(Quantifier.ALL,List(newt2.c),part1)
        val part3 = quanConsts(Quantifier.EX,controllableList,andK & sumK & part2)
        quanConsts(Quantifier.EX,List(newt1.c),part3)
  ...
  def GlobalMC(M:HDMAS_Structure,phi:Formulae,assignment: List[Int]):mutable.Set[State] = phi.getFormulaType match {
    ...
    /* def PrF(M:HDMAS_Structure,s:State,t1:ITerm,t2:ITerm,Q:Set[State]):IFormula = {
      SimpleAPI.withProver { p: SimpleAPI =>
        import p._
        import IExpression._
        val agNum:Int = M.getAgNum
        val actionSet:mutable.Set[Action] = M.getActionSet
        val actionNum: Int = actionSet.size
        val k = createConstants(actionNum, 0 to agNum)
        val l = createConstants(actionNum, 0 to agNum)
        val seq:Seq[Sort] = Seq(0 to agNum)
        for (i <- 1 until actionNum) {seq :+ 0 to agNum}
        val a:IVariable = a
        val b:IFormula = a>5
        val test:IFormula = all
        val formula: IFormula = ex( k(0) => 1 >= k(0) )
      }
...
object ModelChecking{
  def main(args: Array[String]): Unit ={
    ...
    //println(test.GlobalMC(M,phi,null))
    val set1 = test.GlobalMC(M,phi,null)
    println(set1)
    //val set1 = test.G_Fixpoint(M,y1.toPrincess,y2.toPrincess,phi,null,"FE",y1.toPrincess,y2.toPrincess)
    //println(test.PREIMG(M,y1.toPrincess,y2.toPrincess,set1,null,"FE",y1.toPrincess,y2.toPrincess))
    //println(test.PREIMG(M,y1.toPrincess,y2.toPrincess,set1,null,"FE",y1.toPrincess,y2.toPrincess))
